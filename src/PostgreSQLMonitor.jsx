...
// global UI theme used previously as THEME
const THEME = {
  textMain: '#E2E8F0',
  textMuted: '#64748B',
  warning: '#F59E0B',
      ...
      >
        ...
        >
          {type === 'api'
            ...
            : data.problem_query || 'Query plan unavailable.'}
        </div>
      </div>
    </div>
  );
...
const UniversalDBMonitor = () => {
  ...
  const [connectionString, setConnectionString] = useState(
    'postgresql://scott:tiger@localhost:5432/prod_db'
  );
  ...
  const apiQueryData = [
    {
      ...
      calls_per_min: 850,
      db_time_pct: 85,
      queries: [{ sql: 'SELECT count(*)', calls: 1, duration: 120 }],
      ai_insight: 'Heavy aggregation.'
    }
  ];
  ...
  const lowHitRatioData = [
    {
      ...
      ratio: 12,
      total_scans: '5.4M',
      problem_query: "SELECT * FROM logs WHERE msg LIKE '%error%'",
    ...
    !reliabilityViewMode ? (
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 24 }}>
        <GlassCard title="Connections" theme={currentTheme}>
          <div
            onClick={() => setReliabilityViewMode('active')}
          ...
          <div
            onClick={() => setReliabilityViewMode('idle')}
          ...
          >
            ...
            >
              <span>Idle</span>
              <span>{metrics.idleConnections}</span>
            </div>
            <NeonProgressBar
        ...
        <GlassCard title="Errors" theme={currentTheme}>
          {topErrors.map((err, i) => (
            <div
              key={i}
              onClick={() => setReliabilityViewMode('errors')}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          ...
          rightNode={
            <button
              onClick={() => {
                ...
                setSelectedReliabilityItem(null);
              }}
              style={{
        ...
        <GlassCard title="Details" theme={currentTheme}>
          {selectedReliabilityItem ? (
            <div style={{ color: '#fff', whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>
              {JSON.stringify(selectedReliabilityItem, null, 2)}
            </div>
          ) : (
            <EmptyState icon={Search} text="Select Item" />
          )}
        </GlassCard>
      </div>
    );
    ...
    !indexViewMode ? (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
        <MetricCard
          ...
          color={currentTheme.primary}
          theme={currentTheme}
          onClick={() => setIndexViewMode('hitRatio')}
        />
        <MetricCard
          icon={AlertTriangle}
          title="Missing"
          ...
          color={THEME.warning}
          theme={currentTheme}
          onClick={() => setIndexViewMode('missing')}
        />
        <MetricCard
          icon={Layers}
          title="Unused"
          ...
          color={THEME.danger}
          theme={currentTheme}
          onClick={() => setIndexViewMode('unused')}
        />
      </div>
    ) : (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
  ...
  // minimal Resources and Performance tabs so component compiles
  const ResourcesTab = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
      <GlassCard title="CPU" theme={currentTheme}>
        <ResourceGauge
          label="CPU Usage"
      ...
      <GlassCard title="Memory" theme={currentTheme}>
        <ResourceGauge
          label="Memory"
  ...
  const ConnectionModal = () => (
    ...
    >
      <GlassCard
        ...
        theme={currentTheme}
        style={{ width: 600, background: '#0f172a' }}
      >
        <div style={{ marginBottom: 16 }}>
          ...
          >
            <div
              ...
              onClick={() =>
                setConnectionString('postgresql://user:pass@host:5432/db')
              }
            >
            ...
            <div
              style={{ cursor: 'pointer', marginBottom: 4 }}
              onClick={() =>
                setConnectionString('mysql://user:pass@host:3306/db')
              }
            >
            ...
            <div
              style={{ cursor: 'pointer' }}
              onClick={() =>
                setConnectionString('sqlite:///path/to/db.sqlite')
              }
            >
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isSidebarOpen ? (
            <span style={{ fontWeight: 'bold' }}>{dbType} Monitor</span>
          ) : (
            <Database color={currentTheme.primary} />
          )}
          <button
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                color:
                  activeTab === item.id ? currentTheme.primary : '#ccc',
                background:
                  activeTab === item.id
