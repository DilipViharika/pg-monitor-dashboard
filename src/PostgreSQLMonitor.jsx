...
import {
  ...
  Menu, Code, FileText, Network, ArrowRight, Settings, Plug, LogIn, Shield, Mail
} from 'lucide-react';
import {
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div style={{ backgroundColor: 'rgba(2, 6, 23, 0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '12px', boxShadow: '0 0 20px rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)' }}>
        <p style={{ color: '#94A3B8', fontSize: 11, marginBottom: 8, fontFamily: 'monospace' }}>{label}</p>
...
const AIAgentView = ({ type, data }) => {
  if (!data) return (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8', flexDirection: 'column', gap: 12, textAlign: 'center', opacity: 0.6 }}>
      <Terminal size={24} />
      <p style={{ fontSize: 13 }}>Select an item to generate<br/>AI recommendations.</p>
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 16 }}>
      <div style={{ background: 'rgba(168, 85, 247, 0.1)', border: '1px solid rgba(168, 85, 247, 0.4)', borderRadius: 12, padding: 16 }}>
        ...
        <p style={{ fontSize: 13, lineHeight: 1.6, color: '#F8FAFC', margin: 0 }}>
          ...
          {type === 'missing' && <> Table <strong>{data.table}</strong> is experiencing heavy scans on <strong>{data.column}</strong>. Creating an index will reduce I/O by <strong>{data.improvement}</strong>.</>}
          {type === 'unused' && <> Index <strong>{data.indexName}</strong> is <strong>{data.size}</strong> of wasted storage. Last used: <strong>{data.lastUsed}</strong>.</>}
        </p>
      </div>

      <div style={{ flex: 1, background: '#0f172a', borderRadius: 12, border: '1px solid #1e293b', padding: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ background: '#1e293b', padding: '8px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #334155' }}>
          <span style={{ fontSize: 11, color: '#94A3B8', fontFamily: 'monospace' }}>
             {type === 'api' ? 'QUERY_PLAN.sql' : 'SUGGESTED_FIX.sql'}
          </span>
        ...
        <div style={{ padding: 16, fontFamily: "'JetBrains Mono', monospace", fontSize: 13, color: '#a5b4fc', lineHeight: 1.6, flex: 1, overflowY: 'auto' }}>
          {type === 'api' && data.queries.map((q, i) => (
             <div key={i} style={{ borderBottom: '1px solid #334155', paddingBottom: 12, marginBottom: 12 }}>
          ...
          {type === 'hitRatio' && (
            <>
              <span style={{ color: '#64748b' }}>-- Problematic Query:</span><br/>
              {data.problem_query}
            </>
          )}
        </div>
...
const LoginScreen = ({ onLogin }) => {
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', background: '#020617', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#F8FAFC', position: 'relative', overflow: 'hidden' }}>
      {/* Bg Effect */}
      <div style={{ position: 'absolute', top: -100, left: -100, width: 500, height: 500, background: 'radial-gradient(circle, rgba(14, 165, 233, 0.15) 0%, transparent 70%)', borderRadius: '50%' }} />
      ...
      <div style={{ width: 420, padding: 48, background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(20px)', borderRadius: 24, border: '1px solid rgba(56, 189, 248, 0.1)', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)', zIndex: 10 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h2 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>{isLogin ? 'Welcome Back' : 'Create Account'}</h2>
          <p style={{ color: '#94A3B8', fontSize: 14 }}>{isLogin ? 'Enter credentials to access demo' : 'Sign up to start monitoring'}</p>
        </div>
        
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#94A3B8', marginBottom: 8, display: 'block' }}>EMAIL / USER ID</label>
            <div style={{ position: 'relative' }}>
              ...
              <input 
                ...
                style={{ width: '100%', padding: '12px 12px 12px 40px', background: '#1e293b', border: '1px solid #334155', borderRadius: 8, color: 'white', outline: 'none', fontSize: 14 }}
                placeholder="admin"
              />
            </div>
          </div>
          ...
          <button type="submit" style={{ marginTop: 8, padding: '14px', background: '#0EA5E9', color: 'white', border: 'none', borderRadius: 8, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s', fontSize: 14 }}>
            {isLogin ? <LogIn size={18} /> : <UserIcon size={18} />} 
        ...
        <button style={{ width: '100%', padding: '12px', background: 'transparent', border: '1px solid #334155', borderRadius: 8, color: '#F8FAFC', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontSize: 14, fontWeight: 500 }}>
             <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'linear-gradient(135deg, #EA4335 30%, #FBBC05 50%, #34A853 80%, #4285F4 100%)' }}></div>
             Continue with Google
        </button>

        <div style={{ marginTop: 24, textAlign: 'center', fontSize: 13, color: '#94A3B8' }}>
          {isLogin ? "Don't have an account? " : "Already have an account? "}
          <span onClick={() => { setIsLogin(!isLogin); setError(''); }} style={{ color: '#0EA5E9', cursor: 'pointer', fontWeight: 600 }}>{isLogin ? 'Sign up' : 'Log in'}</span>
        </div>
      </div>
    </div>
  );
...
const UniversalDBMonitor = () => {
  ...
  // --- DATA STATES ---
  // (Defined here to be accessible to all tabs)
      ...
      <GlassCard title="Cluster Activity" theme={currentTheme}>
        <ResponsiveContainer width="100%" height={320}>
          <AreaChart data={last30Days} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="qps" stroke={currentTheme.primary} strokeWidth={3} fill="url(#primaryGradient)" filter="url(#neonGlow)" name="QPS" />
          </AreaChart>
        </ResponsiveContainer>
      </GlassCard>
  ...
  // ... (Other Tabs: ResourcesTab, PerformanceTab, etc. - Identical logic as previous code) ...
  const ResourcesTab = () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
        <GlassCard title="CPU" theme={currentTheme}><ResourceGauge label="CPU Usage" value={Math.round(metrics.cpuUsage)} color={THEME.danger} /></GlassCard>
        <GlassCard title="Memory" theme={currentTheme}><ResourceGauge label="Memory" value={Math.round(metrics.memoryUsage)} color={currentTheme.primary} /></GlassCard>
        <GlassCard title="Disk IO" theme={currentTheme}>
        ...
        <GlassCard title="Query Time Distribution" theme={currentTheme}>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={queryTimeDistribution}>
                ...
                <YAxis tick={{ fontSize: 11, fill: THEME.textMuted }} axisLine={false} tickLine={false} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="count" fill="url(#barGradient)" stroke={currentTheme.secondary} strokeWidth={1} />
            </BarChart>
          </ResponsiveContainer>
        </GlassCard>
        <GlassCard title="Uptime & SLA" theme={currentTheme}>
             {/* Simple uptime view */}
             <div style={{ color: '#e5e7eb', fontSize: 12 }}>Uptime: 30d 4h</div>
        </GlassCard>
    </div>
  );
  ...
  const ReliabilityTab = () => (
    !reliabilityViewMode ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            <GlassCard title="List" theme={currentTheme} rightNode={<button onClick={() => { setReliabilityViewMode(null); setSelectedReliabilityItem(null); }} style={{ background: 'transparent', border: 'none', color: '#ccc', cursor: 'pointer' }}>Back</button>}>
  ...
  const IndexesTab = () => (
    !indexViewMode ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            <GlassCard title="List" theme={currentTheme} rightNode={<button onClick={() => { setIndexViewMode(null); setSelectedIndexItem(null); }} style={{ background: 'transparent', border: 'none', color: '#ccc', cursor: 'pointer' }}>Back</button>}>
